...
// 发送缓冲区
uint8_t bus_servo_dma_tx_buf[32] = {0};

void Servo_SendCmd_DMA(uint8_t id, const char *cmd) {
  // 1. 格式化指令（#XXXcmd!，XXX为3位ID）
  memset(bus_servo_dma_tx_buf, 0, sizeof(bus_servo_dma_tx_buf)); // 清空缓冲区
  sprintf((char*)bus_servo_dma_tx_buf, "#%03d%s!", id, cmd);
  uint16_t len = strlen((char*)bus_servo_dma_tx_buf);

  // 2. 等待前一次 DMA 传输完成（避免缓冲区冲突）
  while (HAL_UART_GetState(&huart1) == HAL_UART_STATE_BUSY_TX) {
    // 可添加超时处理，防止死等
  }

  // 3. 启动 DMA 发送
  HAL_UART_Transmit_DMA(&huart1, bus_servo_dma_tx_buf, len);
	HAL_UART_Transmit_DMA(&huart2, bus_servo_dma_tx_buf, len);
}
